...
\cite{BlackHat2004}. They took a very interesting approach to score keeping
especially in relation with their network topology. To prevent teams being able
to directly connecting to the score keeping server they  put the score keeper
...
inform a Local Points Daemon (LPD), also written in Python, a task was completed
by a team so the LPD could generate a new flag, update the task to contain the
new flag, and then report back the new flag. Once the GM received a new flag
from the LPD it would update it in the MySQL database.
...
Since the players would be compiling the service and had root to the server, we
needed a way to check and making sure their service were keeping some form of
necessary base functionality and guarantee the service was running. We did this
by creating a base functionality requirements for each server that would report
to the Score Keeping server the current state of the service. If the base
functionality passed its tests it would report to the score keeper that that
service is good. If it failed it would report that the service did not meet base
functionality would be reported to the score keeper it failed its test cases. If
the service was down, then it would not be able to notify the remote server at
all that the service was up and the score keeper server would default to the
service failing the its cases.
...
whether it was passing or failing its functionality testing. If a service
did not report whether it was passing or failing its test cases it would
default to that the service was down and therefore not passing its test cases.
It would also listen for damage from the Damage Manager and update scores
accordingly. It managed scores and relevant information about the game by
...
see if their opponent's ship was within range and if the service that was
being shot at by the Weapons service was up. If it met both of those cases it
would record it as a hit.
...
prior and encouraged joining ad hoc teams at the competition however most were
...
of administrative system to players could start and stop as they wish. Since
they could work at their own pace and improve their skills it could be added to
as another platform of instruction for the 'Security 101' sessions. 
...
the start of the competition. This impeded some of the video game play aspect of
our intended game play. However it did not stop the game from proceeding.
...
\paragraph*{Players Reaction}
In this competition even though the players stayed for a longer time, all
players appeared to be ill-equipped to participate in this competition. More
importantly they were reluctant to be try and mess around with the services to
